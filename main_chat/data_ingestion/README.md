...
```
main_chat/data_ingestion/service_account_credentials.json
```
...
```bash
0 2 * * * cd /path/to/main_chat/data_ingestion && /path/to/python main_daily_ingestion.py >> logs/cron.log 2>&1
```
...
```batch
@echo off
cd C:\path\to\main_chat\data_ingestion
