...
def extract_pages_from_pdf(file_path: Path) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for i, page in enumerate(reader.pages, start=1):
            ...
            if text:
                pages.append({"page_num": i, "text": text, "total_pages": len(reader.pages)})
        return pages
    except Exception as e:
...
def process_file_to_documents(file_path: Path, file_metadata: Dict[str, Any], chunk_size: int = None, chunk_overlap: int = None) -> List[Document]:
    """
...
def _process_pdf_page_wise(file_path: Path, file_metadata: Dict[str, Any], chunk_size: int, chunk_overlap: int) -> List[Document]:
    """
...
def get_file_info(file_path: Path) -> Dict[str, Any]:
    ...
    """
    stat = file_path.stat()
    return {"name": file_path.name, "size": stat.st_size, "modified": datetime.fromtimestamp(stat.st_mtime).isoformat(), "extension": file_path.suffix.lower()}
