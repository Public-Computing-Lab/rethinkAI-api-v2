...
def reset_database(config_file: Path, recreate: bool = False, sync: bool = False):
    ...
    try:
        ...
        if recreate or sync:
            ...
            from main_chat.data_ingestion.boston_data_sync.boston_data_sync import BostonDataSyncer
...
Examples:
  ...
  python reset_database.py --config custom_config.json --recreate
    ...
    parser.add_argument("--config", type=str, default=None, help="Path to config JSON file (default: boston_datasets_config.json)")
    parser.add_argument("--recreate", action="store_true", help="Recreate tables after dropping (requires --sync to also sync data)")
    parser.add_argument("--sync", action="store_true", help="Sync data after recreating tables (implies --recreate)")
