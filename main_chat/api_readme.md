...
└─────────────────────────────────────────────────────────────────┘
          ...
          ▼                      ▼                      ▼
┌──────────────────┐   ┌──────────────────┐   ┌──────────────────┐
│  sql_retrieval   │   │  rag_retrieval   │   │  Both combined   │
...
# Windows:
main_chat\pocEnv\Scripts\activate
# Linux/Mac:
source main_chat/pocEnv/bin/activate
...
Create a `.env` file in the project root or `main_chat/` directory:
...
The `Config` class in `api.py` manages all configuration:
...
gunicorn -w 4 -b 0.0.0.0:8888 api.api:app
```
...
A test script is provided at `api/test_api.py`:
