...
def _llm_generate_sql(question: str, schema: str, default_model: str, metadata: str = "", conversation_history: List[Dict[str, str]] | None = None) -> str:
    system_prompt = (
        ...
        "- For weekly calendar/event questions, prefer the `weekly_events` table when available. Use `event_name`, `event_date`, `start_date`, `end_date`, `start_time`, `end_time`, and `raw_text` to answer questions such as what events are happening on a given day, what is scheduled this week, or what activities are available.\n"
...
def _llm_refine_sql(
    ...
    system_prompt = (
        ...
        "- For weekly calendar/event questions, prefer the `weekly_events` table when it exists in the schema. Use `event_name`, `event_date`, `start_date`, `end_date`, `start_time`, `end_time`, and `raw_text` to answer questions about what events are happening on specific days or during a given week.\n"
